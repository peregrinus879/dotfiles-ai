...
- Before every commit or publication-readiness assessment, load `/commit`. The skill owns candidate fingerprinting, documentation checks, privacy review, editor instructions, approval routing, staging validation, commit creation, and publication review.
...
- After the final approved commit and any final review, or when evaluating existing commits or artifacts for publication, run `/commit` publication review automatically before presenting a push, release, pull request, or other publication as ready. Use an unambiguous configured upstream and the default world-readable audience without asking H to restate them; ask only when destination or audience is materially ambiguous.
