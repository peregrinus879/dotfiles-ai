...
4. Present the packet: `git diff --cached --stat`, the proposed message, the tree, parent, and branch, verification results and anything not run, the privacy screen result, and the scratch disposition. Do not paste the full diff unless H asks. Tell H to review with `git diff --cached` or Neovim's staged-hunk view, then offer `Commit and resume`, `Commit and pause`, and free-text revision, rejection, or questions.
