...
Requires: the `codex` CLI and the stowed `spar-codex` bridge (this repo, codex/.local/bin), which hard-codes the safe flags: exact ChatGPT-subscription and effective-plugin-disable preflights; higher-precedence authentication plus caller-directed state, certificate, and routing overrides rejected; privileged Bash startup with environment startup files disabled; one reviewer executable resolved before changing directories; ignored user config and rules; strict config; launch from the untrusted handoff root with non-Git execution allowed; one inline profile denying root and temp access while allowing runtime-minimal paths, the resolved Codex executable, and the handoff, with network disabled and a secret-filtered shell environment; max effort with reasoning summaries disabled; MCP, apps, local and remote plugins, hooks, web search, multi-agent, skill-catalog, skill-install, and shell-snapshot surfaces disabled; stdin guard; a 180 s stall watchdog under a 1800 s ceiling; fail-fast usage-limit classification; bounded payload-scanned stderr diagnostics; failed-new thread reporting; never `--last`. The trusted global `~/.codex/AGENTS.md` remains because ChatGPT authentication and resumable sessions use the normal `CODEX_HOME`. Tracked permissions auto-approve `Bash(spar-codex *)`; the pinned reviewer can read the handoff without gaining write access.
...
- New review: run `spar-codex new "$HANDOFF" "<prompt>"` plainly, never via command substitution (allow rules do not match `$(...)` forms); the reply text arrives on stderr, the thread id on stdout, and the bridge appends five-field timestamp, bridge, role, state (`started`, `completed`), and id lines to its reserved `reviewer-id` manifest; state the literal thread id in the user presentation. If the fresh thread starts but the call later fails, the manifest and the `SPAR-BRIDGE THREAD` stderr line preserve the id. A deliberately independent one-pass review may use the trailing role argument `cold` and is never resumable.
- Follow-up review: `spar-codex resume <id> "$HANDOFF" "<pointer prompt>"`; the reply is stdout, and the bridge accepts only a `primary` id recorded for Codex in that handoff.
