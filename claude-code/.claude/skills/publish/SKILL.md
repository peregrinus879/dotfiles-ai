...
3. Run `git diff <base-id> <reviewed-id> | spar-payload-scan diff`, and for a delta with later-deleted paths also `git log -p --format= <base-id>..<reviewed-id> | spar-payload-scan diff`; a finding blocks readiness until H rules on it. Then review every commit in the delta: message, author identity, and every changed path's content, including files later deleted, for the commit skill's privacy classes. Report only metadata for binary or unreadable files and ask H to inspect them in a separate pane. Any unresolved finding blocks readiness.
