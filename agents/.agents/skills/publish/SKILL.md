...
5. Present the binding, the commit list with ids, the review result, and the script's command:
   ...
   The `-C` argument names the repository root, so the command runs unchanged from whatever directory H's terminal is in. The lease makes the destination reject the push whenever its branch is not exactly the reviewed base, including a branch someone deleted or moved after the review, and it forces nothing when the base matches; it is the guard the review depends on, not a bypass. Any change to the bound values invalidates the result: rerun `publish-bind`. H runs the command. Follow it with the tool's interactive question selector, `Pushed, verify and resume` first and `Pushed, verify and pause` second, with the command repeated in the selector's question, since the selector can hide the message above it. Free text carries a question or a comment: answer it, then present the command and the selector again. Only a selected option starts the verification; pasted push output is not needed.
6. After the push, run `~/.agents/skills/publish/scripts/publish-verify`. It confirms the tracking ref equals the reviewed id and the branch is neither ahead nor behind, then runs the repository's published verification when it defines one, `make verify-published REV=<id>` or `npm run verify-published -- <id>`, which waits for the deployment behind the destination and confirms the published state carries the reviewed commit. Report the pushed id and the result; a repository without the target is reported as pushed without published verification. When the tracking ref does not carry the reviewed id, the push did not land: report that exactly and present the command and the selector again. Any other state is reported exactly and stops for H's decision. Then report completion: `resume` continues the request's remaining work, `pause` stops for discussion.
