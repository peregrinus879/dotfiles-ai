...
2. Round 0, blind sketch: send the reviewer only the goal and constraints, inline and never in any plan-bearing file (more than a few paragraphs goes in a blind handoff file without the plan), and ask for a 3-5 bullet independent approach sketch; capture the pinned session id. This exists to prevent anchoring on the author's framing.
...
7. Converged or capped, stop and present to the user before any execution: the final plan; enough factual context for a ruling on every open or disputed item; one concise paragraph per side stating its rationale and consequence; the implementer's one-line recommended ruling labeled judgment; and the reviewer session ids for interactive handoff. Never present only option labels or delegate synthesis or the ruling to the reviewer. Convergence between the models is not correctness, and disagreement surviving the cap is signal for the user, not failure. Execute only on the user's go.
