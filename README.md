...
Codex, OpenCode, and `spar-codex` use GPT-5.6 Sol Fast at xhigh reasoning. The managed configuration assumes ChatGPT subscription authentication, where Fast consumes GPT-5.6 credits at 2.5 times the Standard rate. API-key use invokes separately billed processing and requires a separate configuration and billing decision.
