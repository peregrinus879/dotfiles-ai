...
Before authentication or network access, each bridge scans the prompt and complete handoff, rejects alternate authentication plus caller-directed state and routing, validates one private mode-700 `/var/tmp/spar-<session-id>/` directory, and resolves the reviewer executable once before changing directories. Both launch the reviewer from the handoff, flush it before every call, and accept resume only when the bridge-owned manifest records that primary session for the same bridge and handoff; bridge-specific legacy pre-state entries keep in-flight four-field manifests resumable. Cold sessions never resume. `spar-codex` treats the handoff as untrusted, suppresses project instructions and skill catalogs, disables plugins, and preflights an empty effective plugin inventory. It retains the trusted global `~/.codex/AGENTS.md` because normal `CODEX_HOME` supplies ChatGPT authentication and resumable state. Its inline profile denies root and temp access while allowing runtime-minimal paths, the resolved Codex executable, and the handoff. `spar-claude` selects the current `opus` alias, uses safe mode with no setting sources, discovered `CLAUDE.md`, or auto memory, combines `dontAsk` with one absolute handoff `Read` allow, exposes only `Read`, `Glob`, and `Grep`, validates every served-model key as Opus-family, and disables nonessential traffic and updater activity. A call succeeds only after one valid terminal event with a nonempty reply, and a new Codex call also requires a valid thread ID. On reviewer-process failure, `spar-codex` scans bounded captured stderr before relaying it, classifies stderr-only limits, and reports any valid new thread ID created before failure. Each bridge owns the reviewer process group so stall and ceiling exits terminate TERM-ignoring descendants before returning.
