...
- OpenCode subagent delegation (source-checked 2026-09-04 on 1.18.26, `tool/task.ts`): the task tool checks the `task` permission against the requested subagent type and enforces `subagent_depth`, default 1, so a subagent cannot launch another; the `reviewer` agent denies `task` as well. Recheck when that source changes.
...
- WSL host pass, for the agent to run in a session opened in the WSL clone, stopping at the first mismatch:
  ...
  - After the same `make restow`, expect the `reviewer` agents at `~/.claude/agents/reviewer.md` and `~/.config/opencode/agents/reviewer.md`, the retired `~/.config/opencode/skills` links removed by `make clean` and their emptied directories to be deleted by hand, the host Codex config carrying `personality = "none"` and the `[hooks]` table, and OpenCode reading skills through `skills.paths` and lightweight tasks through `small_model`; restart OpenCode.
