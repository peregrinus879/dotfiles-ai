...
- Observed 2026-09-03 on Codex 0.152.1: a literal denied workspace path such as `.env` makes the sandbox create an empty mode-444 placeholder file in every workspace where the path is absent, and a home-wide glob that matches a file inside an already denied directory (`~/**/id_ed25519` under `~/.ssh`) aborts sandbox startup. Profiles therefore use `**/` globs in the workspace and deny directories, not their contents, under `$HOME`; recheck both behaviors when the sandbox changes.
