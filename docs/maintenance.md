...
- Rename the skills with an `eyr` prefix so they cannot collide with built-in or plugin commands; the form is undecided, H preferring the joined `eyrcommit` over `eyr-commit`. The rename touches the three skill directories and their Codex and OpenCode symlinks, the OpenCode command wrappers, the Codex template read grants, the `tests/prepare-stow.sh` fixture, and every cross-reference; deploy with `make clean`, `make restow`, and `make verify`, then remove the emptied skill directories under `$HOME`.
- Roll the gate contract into the sibling repositories, one commit each in its own session, leaving omasecboot until H's current work there is done: eyrarchy adds `test`, `require-host`, and `require-clone` and folds `lint` and `check` into `verify`; eyrwsl adds `require-clone`, folds `lint` and `check` into `verify`, and trims the omasync completion check to `make wt-diff`; both delete the verification sentences from `AGENTS.md` and add tracked symlinks so Codex (`.agents/skills`) and OpenCode (its project skill directory, confirmed against the source clone under `~/Projects/quarry`) load the project omasync skill; the private website repository adds `check` and `verify` scripts, renders its build commit so a `verify-published` script can poll the production URL for it, and deletes its conditional verification rule.
