...
Requires: the `claude` CLI, `jq`, and the stowed `spar-claude` bridge (this repo, claude-code/.local/bin), which hard-codes the safe flags: exact first-party claude.ai-subscription preflight with higher-precedence authentication plus caller-directed state, certificate, and routing overrides rejected; privileged Bash startup with environment startup files disabled; one reviewer executable resolved before launch from the handoff; an explicit `Read`, `Glob`, and `Grep` tool surface combined with `dontAsk`, disallowed write and shell tools, and one absolute handoff `Read` allow; safe mode with no setting sources, discovered `CLAUDE.md`, or auto memory; the latest permitted Opus family with its alias override cleared; terminal served-family validation; prompts via stdin; a stall watchdog under an absolute ceiling (SPAR_BRIDGE_STALL and SPAR_BRIDGE_CEILING, positive integers only, defaults 180/1800); and fail-fast usage-limit classification.
...
- New review: run `spar-claude new "$HANDOFF" "<prompt>"` plainly, never via command substitution; the reply text arrives on stderr, the session id on stdout, and the bridge appends five-field timestamp, bridge, role, state (`allocated`, `completed`), and id lines to its reserved `reviewer-id` manifest; state the literal session id in the user presentation. A deliberately independent one-pass review may use the trailing role argument `cold` and is never resumable.
- Follow-up review: `spar-claude resume <id> "$HANDOFF" "<pointer prompt>"`; the reply is stdout, and the bridge accepts only a `primary` id recorded for Claude in that handoff.
