...
- Codex: the root-denied `trusted-workspace` profile with automatic approval review, managed runtime reads, workspace and OS-temp writes, credential denies, and command network disabled. `templates/codex/config.toml` is the portable profile; `make migrate-codex-config` installs it as a host-local `~/.codex/config.toml` that Codex and the desktop app rewrite freely, so host state never enters Git. Skills live under `~/.agents/skills`. Scripted `codex exec` calls use one finite prompt and never `--last`.
- OpenCode: ordinary workspace edits and shell commands are allowed; native filesystem access outside the app temp root asks; pushes, repository-host mutations, destructive Git operations, privilege escalation, remote shells and transfers, and nested agent launches are denied; sharing is disabled. Permission order is last-match-wins. Read and edit subjects are worktree-relative and external subjects are parent directories, so credential stores under `$HOME` are guarded by the external-directory globs and the ask default, and `apply_patch` checks every target against those rules before writing. OpenCode has no classifier or sandbox, so its Bash rules are guardrails, not containment. Automatic external skill discovery stays disabled.
