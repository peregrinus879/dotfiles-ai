...
- Claude Code and `spar-claude` pin Fable 5 with xhigh effort; the reviewer bridge also disables nonessential traffic and updater activity.
...
- Spar handoffs use one private mode-700 `/var/tmp/spar-<session-id>/` directory per session on disk-backed OS temp, so an in-flight review survives reboots; both pinned read-only reviewers can read that exact directory under their sandboxes, with `spar-codex` granting its reviewer exactly the validated handoff directory. The selected reviewer bridge creates the directory, revalidates ownership, mode, containment, symlinks, and link counts before every review call and again before cleanup, and flushes the handoff filesystem before every review call; the same durability step is its `flush` mode, run after every native handoff write.
