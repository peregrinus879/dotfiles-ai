...
- spar runs prompt-free through tracked allow rules (2026-08-12, user-directed): `Bash(timeout 600 opencode run *)` plus Edit/Write on `./spar-scratch*/**`; the first-round `SID=$(...)` capture still prompts (command-substitution forms never match allow rules, anthropics/claude-code#43713), and a bare `opencode run` allowed by the rule is contained by OpenCode's own ask map, whose asks hang headless runs rather than execute
...
- watch native cross-model features in both tools for spar-relevant capabilities: gap analysis re-run 2026-08-12 (Claude Code agent teams experimental and Anthropic-only; OpenCode 1.18.16 ships no native review), bridge kept; re-run when either ships a native reviewer or cross-vendor path; the `claude -p` bridge is kept partly because it rides subscription auth rather than API-key billing
...
- spar bridge visibility: when the spar runs inside tmux, prototype opening a tmux window named after the counterpart tool attached to the pinned reviewer session (`opencode -s "$SID"` / `claude -r "$sid"`) so rounds are watchable live; first live datum 2026-08-12: a standalone TUI attached with `opencode -s` showed the session only as of open and did not stream concurrent headless updates (reopen required), so the prototype likely needs the shared-server path (`opencode serve` plus `run --attach`) rather than a second standalone client
