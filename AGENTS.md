...
- spar runs prompt-free through tracked allow rules (2026-08-12, user-directed): `Bash(timeout 600 opencode run --agent reviewer *)` plus Edit/Write on `./spar-scratch*/**`; the first-round `SID=$(...)` capture still prompts (command-substitution forms never match allow rules, anthropics/claude-code#43713); the rule pins the read-only reviewer agent, whose edit and bash denies hold even if `--auto` is appended
