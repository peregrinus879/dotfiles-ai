...
- headless `opencode run` has two live-verified hang classes (2026-08-12, 1.18.16): argv prompts above ~3.8 KB hang before session creation, and any permission resolving to ask (including `external_directory` reads) hangs forever; the spar skill therefore keeps payloads in an in-repo `spar-scratch/` handoff directory, a recorded exception to the session-scratch rule, and caps argv prompts at ~2 KB
...
- the spar reviewer stays frontier-tier and cross-vendor by design (a weaker cross-vendor reviewer measurably degrades a stronger author's output; arXiv 2607.21656: +18.1pp stronger, -8.6pp weaker); the charter deliberately never names the authoring model (source anonymity reduces identity-driven sycophancy), and a same-model lens swarm was evaluated 2026-08-12 and rejected (correlated same-family errors yield few effective votes)
...
- if `claude -p` ever defaults to bare mode, re-verify the OpenCode-side spar bridge auth: bare mode never reads OAuth credentials, which would silently drop the bridge off subscription auth
...
- spar bridge visibility: when the spar runs inside tmux, prototype opening a tmux window named after the counterpart tool attached to the pinned reviewer session (`opencode -s "$SID"` / `claude -r "$sid"`) so rounds are watchable live; needs a live test for two-client behavior on one session
